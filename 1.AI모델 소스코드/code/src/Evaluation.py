...
class Evaluation():
    ...
    def load_model(self, model, weight_path):
        ...
        try:
            model.model.load_state_dict(torch.load(weight_path, map_location=torch.device(self.device), weights_only=False))
            return model.model
        except:
            model.load_state_dict(torch.load(weight_path, map_location=torch.device(self.device), weights_only=False))
            return model
    ...
    def validation(self, model, epoch, step, data_loader):
        ...
        with torch.no_grad():
            ...
            if self.one_channel:
                ...
                message='Validation_label #{} #{} Average Loss: {:.4f}, mIoU: {:.4f}, background IoU : {:.4f}, target IoU : {:.4f}'.format(epoch+1, step, avrg_loss, mIoU, cls_IoU[0], cls_IoU[1] )
            else:
